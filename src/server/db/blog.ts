import { ICredsFound } from "../types/types";
...
export const checkAuthor = async (name: string) => {
  return await Query("SELECT * from Authors WHERE email = ?", [name]);
};
...
export const findCredentials = async (email: string) => {
  return <ICredsFound[]>(
    await Query("select * from Credentials where email = ?", [email])
  );
};

export const getPersonalBlogs = async (authorid: any) => {
  return await Query(
    "select a.id as userID, a.name as name, b.id as blogID, b.title as blogTitle, b.content as content, t.name as tag, t.id as tagID from Blogs b join Authors a on a.id = b.authorid left join BlogTags bt on b.id = bt.blogid left join Tags t on t.id = bt.tagid where a.id = ?",
    [authorid]
  );
};

export const getAuthor = async (userID: any) => {
  return Query("select name from Authors where id = ?", [userID]);
};
