...
//       check authors
router.get("/api/check/authors/:email", async (req, res) => {
  try {
    let email = req.params.email;
    let authorCheck = await db.Blog.checkAuthor(email);
...
//    personal blogs
router.get("/api/blogs/authors/tags/:authorid", async (req, res) => {
  try {
    let authorid = req.params.authorid;
    let personalBlogs = await db.Blog.getPersonalBlogs(authorid);
...
//    get auther name
router.get('/get/author/from/id/:id', async(req, res) => {
  try {
    let userid = req.params.id;
    let user = await db.Blog.getAuthor(userid)
    res.json(user)
  } catch (error) {
    console.log(error)
    res.sendStatus(500)
...
router.post("/api/post/author", async (req, res) => {
  try {
    ...
    let newAuthor = <{ insertId: number }>await db.Blog.postAuthor(name, email);
    console.log(newAuthor);
    console.log(`Author, ${name}, added to db`);
    res.status(200).json(newAuthor.insertId);
  } catch (e) {
    if (e.code === "ER_DUP_ENTRY") {
      console.log("*")
      console.log(e);
      console.log("*")
      res.json({ message: "acct taken" });
    } else {
      console.log(e);
