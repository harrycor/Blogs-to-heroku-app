...
import { isConditionalExpression, resolveModuleName } from "typescript";
...
const NewPost = () => {
  ...
  useEffect(() => {
    ...
    apiService(`/auth/access`).then((res) => {
      ...
      // console.log(res)
      apiService(`/get/author/from/id/${res.userid}`).then((res) =>{
        console.log(res)
        setAuthorId(res[0].name)
  ...
  let submitFunc = () => {
    ...
    if (!authorId) return alert("You didnt enter your name dummy!");
      ...
      console.log(userEmail + "11111")
      fetch(`/api/check/authors/${userEmail}`)
        .then((res) => res.json())
        .then((res) => {
          console.log(res)
          if (res[0].id) {
            ...
            fetch(`/api/postblog`, {
              ...
              .then(dataLastId => {
                ...
                if (tag) {
                  ...
                  //checks for tag
                  apiService(`/api/gettag/${tag}`)
                    .then((res) => {
                      console.log(res + "***");
                      if (res[0]) {
                        tagIDVar = res[0].id;
                        fetch(
                          `/api/postblogtag/${dataLastId}/${tagIDVar}`,
                          {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                          }
                        ...
                        fetch(`/api/post/newtag/${tag}`, {
                          ...
                          .then((resNum) => {
                            console.log("44");
                            fetch(
                              `/api/postblogtag/${dataLastId}/${resNum}`,
                              {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                              }
